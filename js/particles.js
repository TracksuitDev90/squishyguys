...
export function emitMerge(x, y, tierIndex, comboCount) {
  ...
  // Shockwave ring(s)
  shockwaves.push({
    x, y, startRadius: r * 0.3, endRadius: r * 3.5,
    color, lineWidth: 4 + tierIndex * 0.5,
    life: 1, decay: 0.025 + intensity * 0.005,
  });
  // Second thinner ring for higher tiers
  if (tierIndex >= 4) {
    shockwaves.push({
      x, y, startRadius: r * 0.5, endRadius: r * 4.5,
  ...
  // Sparkles burst
  const sparkleCount = 10 + tierIndex * 3;
  for (let i = 0; i < sparkleCount; i++) {
    const angle = (Math.PI * 2 * i) / sparkleCount + (Math.random() - 0.5) * 0.5;
    const speed = (2 + Math.random() * 4) * (1 + intensity * 0.8);
    sparkles.push({
      x, y,
      vx: Math.cos(angle) * speed,
      vy: Math.sin(angle) * speed - 1.5,
      size: 2 + Math.random() * 3 + tierIndex * 0.3,
      color,
      life: 1,
  ...
  // White flash sparkles
  for (let i = 0; i < 5 + tierIndex; i++) {
  ...
  // Confetti for higher tiers (blue+)
  if (tierIndex >= 5) {
    const confettiCount = 6 + (tierIndex - 5) * 4;
    ...
    for (let wave = 0; wave < 3; wave++) {
      setTimeout(() => {
        shockwaves.push({
          ...
          color: `hsl(${wave * 120}, 80%, 65%)`, lineWidth: 3,
          life: 1, decay: 0.018,
        });
      }, wave * 100);
    }
  ...
  // Screen shake (bigger for higher tiers)
  const shakeAmount = 3 + tierIndex * 1.5 + comboCount * 2;
